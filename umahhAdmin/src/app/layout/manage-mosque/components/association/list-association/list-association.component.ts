...
export class ListAssociationComponent implements OnInit {
  ...
  deleteAssociation() {
    ...
    this.isLoading = true;
    this.manageMosqueService.deleteAssociation(newObject).subscribe(res => {
  ...
  getAssociationList() {
    ...
    this.manageMosqueService.getAssociationList(this.loggedInId, data).subscribe(res => {
      this.isLoading = false;
      if (res.code == 200) {
        this.mosqueAssociationList = res.data.associate_user;
