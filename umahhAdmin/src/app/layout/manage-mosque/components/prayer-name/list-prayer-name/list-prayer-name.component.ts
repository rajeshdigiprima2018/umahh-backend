...
@Component({
  selector: 'app-list-prayer-name',
  templateUrl: './list-prayer-name.component.html',
  styleUrls: ['./list-prayer-name.component.scss'],
  animations: [routerTransition()]
})
export class ListPrayerNameComponent implements OnInit {
  ...
  deletePrayerName() {
    this.isLoading = true;
    this.manageMosqueService.deletePrayerNameById(this._id).subscribe(res => {
  ...
  getPrayerNameList() {
    ...
    this.manageMosqueService.getPrayerNameList(this.loggedInId, data).subscribe(res => {
      this.isLoading = false;
      if (res.code == 200) {
        this.prayerNameList = res.data;
