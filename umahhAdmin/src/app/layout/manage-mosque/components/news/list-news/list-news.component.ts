...
export class ListNewsComponent implements OnInit {
  ...
  deleteNews() {
    this.isLoading = true;
    this.manageMosqueService.deleteNews(this._id).subscribe(res => {
  ...
  getNewsList() {
    ...
    this.manageMosqueService.getNewsList(this.loggedInId, data).subscribe(res => {
      this.isLoading = false;
      if (res.code == 200) {
        this.mosqueNewsList = res.data;
