...
@Component({
  selector: 'app-list-supplication',
  templateUrl: './list-supplication.component.html',
  styleUrls: ['./list-supplication.component.scss'],
  animations: [routerTransition()]
})
export class ListSupplicationComponent implements OnInit {
  ...
  deleteSupplication() {
    this.isLoading = true;
    this.manageMosqueService.deleteSupplication(this._id).subscribe(res => {
  ...
  getSupplicationList() {
    ...
    this.manageMosqueService.getSupplicationList(this.loggedInId, data).subscribe(res => {
      this.isLoading = false;
      if (res.code == 200) {
        this.SupplicationList = res.data;
