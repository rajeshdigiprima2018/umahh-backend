...
@Component({
  selector: 'app-list-khutba',
  templateUrl: './list-khutba.component.html',
  styleUrls: ['./list-khutba.component.scss'],
  animations: [routerTransition()]
})
export class ListKhutbaComponent implements OnInit {
  ...
  deleteKhutba() {
    this.isLoading = true;
    this.manageMosqueService.deleteKhutba(this._id).subscribe(res => {
  ...
  getKhutbaList() {
    ...
    this.manageMosqueService.getKhutbaList(this.loggedInId, data).subscribe(res => {
      this.isLoading = false;
      if (res.code == 200) {
        this.mosqueKhutbaList = res.data;
