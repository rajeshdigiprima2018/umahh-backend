...
export class UserComponent implements OnInit {
    ...
    getUser() {
        ...
        this.userService.getUserListing(this.dataRole).subscribe(res => {
            if (res.code == 200) {
                // this.user = res.data;
                this.user = [];
                for (var i = 0; i < res.data.length; i++) {
                    if (this.loggedInUserDetails.role == 'mosque') {
                        if (this.loggedInUserDetails.user_id === res.data[i].mosque_id) {
    ...
    deleteUser() {
        this.isLoading = true;
        this.userService.deleteUser(this._id).subscribe(res => {
    ...
    /*
          updateStatus(){
            this.displayUpdateStatus=false;
            this.supportAdminService.updateUserStatus(this.updateStatusData).subscribe(res => {
                  this.isLoading = false;
                  if(res.code==200){
                    this.toastr.success(res.message);
                    this.searchUser();
